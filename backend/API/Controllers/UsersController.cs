...
namespace API.Controllers
{
  ...
  public class UsersController : BaseApiController
  {
    ...
    public async Task<ActionResult> SetMainPhoto(int photoId)
    {
      var user = await _userRepository.GetUserByUsernameAsync(User.GetUserName());
      if (user == null) return NotFound();
      var photo = user.Photos.FirstOrDefault(img => img.Id == photoId);
      if (photo == null) return NotFound();
      if (photo.IsMain) return BadRequest("Already your main photo");
      var currentMainPhoto = user.Photos.FirstOrDefault(img => img.IsMain);
    ...
    public async Task<ActionResult> DeletePhoto(int photoId)
    {
      ...
      return BadRequest("Problems with deleting photo");
    }
  }
}